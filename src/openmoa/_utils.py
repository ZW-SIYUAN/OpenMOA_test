...
def _translate_metric_name(metric_name, to="openmoa"):
    ...
    if to == "moa":
        ...
        raise ValueError("Invalid translation direction. Use 'moa' or 'openmoa'.")
...
def _leaf_prediction(leaf_prediction):
    ...
    See :py:class:`openmoa.classifier.EFDT` and :py:class:`openmoa.classifier.HoeffdingTree`
    ...
    if isinstance(leaf_prediction, str):
        leaf_prediction_mapping = {
            "MajorityClass": 0,
            "NaiveBayes": 1,
            "NaiveBayesAdaptive": 2,
        }
        leaf_prediction = leaf_prediction_mapping.get(leaf_prediction, None)
    if (
        not isinstance(leaf_prediction, int)
        or leaf_prediction < 0
        or leaf_prediction > 2
    ):
        raise ValueError(
            "Invalid value for leaf_prediction, valid options are 'MajorityClass' or 0, "

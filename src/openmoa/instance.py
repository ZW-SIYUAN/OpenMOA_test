...
class LabeledInstance(Instance):
    ...
    with the class label and index. For example, the :class:`openmoa.datasets.ElectricityTiny`
    dataset:
...
class RegressionInstance(Instance):
    ...
    for you with the target value. For example, the :class:`openmoa.datasets.Fried`
    dataset:

    >>> from openmoa.datasets import Fried
    ...
    >>> from openmoa.instance import RegressionInstance
    >>> stream = Fried()

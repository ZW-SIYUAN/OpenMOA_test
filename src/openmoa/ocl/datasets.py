...
>>> from openmoa.ocl.datasets import SplitMNIST
>>> scenario = SplitMNIST(preload_test=False)
>>> scenario.task_schedule
[{1, 4}, {5, 7}, {9, 3}, {0, 8}, {2, 6}]


To get the usual OpenMOA stream object for training:
...
OpenMOA streams flatten the data into a feature vector:
...
from openmoa.datasets import get_download_dir
from openmoa.instance import LabeledInstance
from openmoa.ocl.util.data import class_incremental_schedule, partition_by_schedule
from openmoa.stream import Stream, TorchClassifyStream
...
class _BuiltInCIScenario(ABC):
    ...
    def __init__(
        ...
        :param directory: The directory to download the dataset to, defaults to
            :func:`openmoa.datasets.get_download_dir`.
...
class TinySplitMNIST(_BuiltInCIScenario):
    ...
    mean = [0.1307]
    std = [0.3081]
    url = "https://www.dropbox.com/scl/fi/7ipn2j3r7okbtwyx7npi0/openmoa_tiny_mnist.npz?rlkey=4ujpsgu18s217hmqrka6acc9x&st=r3lehylt&dl=1"
    ...
    def _download_dataset(
        ...
        filename = directory / "openmoa_tiny_mnist.npz"

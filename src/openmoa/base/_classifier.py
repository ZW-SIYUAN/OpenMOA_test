...
from openmoa.type_alias import LabelIndex, LabelProbabilities
...
class BatchClassifier(Classifier):
    ...
    - :func:`openmoa.ocl.evaluation.ocl_train_eval_loop`
    - :func:`openmoa.evaluation.prequential_evaluation`
    ...
    >>> from openmoa.base import BatchClassifier
    ...
    >>> from openmoa._utils import batched # Not available in python < 3.12
    ...
    batch variants with a batch of size 1. This is useful for parts of OpenMOA
...
class MOAClassifier(Classifier):
    """
    A wrapper class for using MOA (Massive Online Analysis) classifiers in OpenMOA.
...
class SKClassifier(Classifier):
    """A wrapper class for using scikit-learn classifiers in OpenMOA.

    Some of scikit-learn's classifiers that are compatible with online learning
    have been wrapped and tested already in OpenMOA (See :mod:`openmoa.classifier`).
    ...
    See also :class:`openmoa.base.SKRegressor` for scikit-learn regressors.
    """

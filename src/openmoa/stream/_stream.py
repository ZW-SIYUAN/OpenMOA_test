...
class Schema:
    ...
    When working with datasets built into OpenMOA (see :mod:`openmoa.datasets`)
    ...
    def from_custom(
        ...
        """Create a OpenMOA Schema that defines each attribute in the stream.
        ...
        :return CayMOA Schema: Initialized OpenMOA Schema which contain all
...
class MOAStream(Stream[_AnyInstance]):
    ...
    def __init__(
        ...
        Usually, you will want to construct a Stream using the :func:`openmoa.stream.stream_from_file`
        function.
...
class LibsvmStream(Stream):
    ...
    def _create_schema(self) -> Schema:
        """Create OpenMOA Schema for this stream."""
        attributes = FastVector()
    ...
    def _parse_libsvm_line(self, line: str, instance_id: int) -> Union[LabeledInstance, RegressionInstance]:
        """Parse LIBSVM line into OpenMOA Instance with sparse features."""
        parts = line.split()
        label = float(parts[0])

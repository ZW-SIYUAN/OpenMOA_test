...
class _ShrubEnsembles(ABC):
    ...
    **Note:**
    This class should not be instantiated directly, but as it only implements the base algorithm. For classification tasks use :class:`openmoa.classifier.ShrubsClassifier` and for regression tasks use :class:`openmoa.regressor.ShrubsRegressor`.

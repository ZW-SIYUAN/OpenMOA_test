"""capymoa/classifier/_oasf_classifier.py"""
...
class OASFClassifier(Classifier):
    """Online Active Sparse Feature learning (OASF) Classifier.
    
    Handles online learning with dynamically evolving feature spaces where features
    can appear (incremental) or disappear (decremental) over time. Uses ℓ1,2-norm
    regularization for sparsity and passive-aggressive updates.
    ...
    >>> evolving_stream = EvolvingFeatureStream(base_stream, d_min=2, d_max=8)
    ...
    def __init__(
        ...
        """Initialize OASF Classifier.
        
        :param schema: The schema of the stream
        :param lambda_param: Regularization parameter for ℓ1,2-norm sparsity
        :param mu: Penalty parameter for PA update (larger = more rigid)
        ...
        # Feature matrix for CUR decomposition
        ...
        # Selected columns from CUR (for active learning)
        self.selected_columns = []

    def __str__(self):
        return (f"OASFClassifier(lambda={self.lambda_param}, mu={self.mu}, "
    ...
    def train(self, instance: Instance):
        ...
        # Apply ℓ1,2-norm sparsification (Theorem 3)
        self._apply_l12_sparsity()
        
        # Update feature matrix for CUR
        self.X_cur = np.roll(self.X_cur, -1, axis=1)
        self.X_cur[:, -1] = 0
        self.X_cur[:d_current, -1] = x_full
        
        # Perform CUR decomposition periodically
        if self.t % self.L == 0:
            self._update_cur()
    ...
    def _update_decremental(self, xt: np.ndarray, yt: float, d_current: int) -> np.ndarray:
        """Update for decremental case (Theorem 1)."""
    ...
    def _update_incremental(self, xt: np.ndarray, yt: float, d_current: int) -> np.ndarray:
        """Update for incremental case (Theorem 2)."""
    ...
    def _apply_l12_sparsity(self):
        """Apply ℓ1,2-norm regularization for sparsity (Theorem 3)."""
    ...
    def _update_cur(self):
        """Update CUR decomposition for active learning (simplified)."""
        # Simple column selection based on importance

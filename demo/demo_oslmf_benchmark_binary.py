"""
demo_oslmf_benchmark_binary.py
------------------------------
The ULTIMATE OpenMOA Benchmark Runner - OSLMF Edition
-----------------------------------------------------
Target: Binary Classification (OSLMF)
Protocol Alignment:
...
5. [Safety] RCV1 skipped due to O(d^2) memory.
"""
...
try:
    from capymoa.classifier._oslmf_classifier import OSLMFClassifier
...
# === Configuration ===
N_REPEATS = 10
OUTPUT_DIR = "./experiments_oslmf_binary"
BURN_IN_SIZE = 500
...
class AutoTuner:
    """Adaptive Auto-Tuner for OSLMF Learning Rate."""
...
def plot_and_save_trace(agg_df, mode, d_name, output_dir):
    ...
    plt.title(f"OSLMF: {d_name} - {mode} (Avg LR={avg_lr:.3f})", fontsize=14)
...
def main():
    ...
    print(f"🚀 Starting OSLMF Benchmark")
    ...
    for mode in modes:
        ...
        for d_name, d_cls in datasets:
            ...
            if d_name in OOM_RISK_DATASETS:
                print(f"[{curr_comb}/{total_combinations}] {d_name:<15} | SKIPPED (OOM Risk for OSLMF)")
                continue

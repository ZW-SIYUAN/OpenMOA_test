...
-----------------------------
The ULTIMATE OpenMOA Benchmark Runner (FTRL Edition)
----------------------------------------------------
Target: Binary Classification (FTRL)
Strictly matches the logic of demo_fobos_benchmark_binary.py:
1. [Auto-Tuning] Adaptive burn-in size (safe for small datasets).
2. [Shuffling] Correctly shuffles static datasets to prevent label sorting bias.
3. [Safety] Robust stream length detection.
...
OUTPUT_DIR = "./experiments_ftrl_binary"  # [CHANGE] Output dir
...
class AutoTuner:
    ...
    def tune(self, stream_builder_func, schema, base_seed, n_total):
        ...
        for alpha in self.alphas:
            ...
            # [CHANGE] Use FTRLClassifier with FTRL params
...
def main():
    ...
    print(f"🚀 Starting Binary Benchmark (FTRL Edition)")

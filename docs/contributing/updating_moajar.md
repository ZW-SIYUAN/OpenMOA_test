# Updating OpenMOA's `moa.jar` version

This document describes how to change the version of MOA that the OpenMOA
project uses. **It is only intended for developers who are contributing to
OpenMOA**. Before you start, make sure you have the following:
...
When a developer wants to replace the `moa.jar` in their local OpenMOA as a
...
When a developer needs to update the version of MOA that the openmoa project uses
they need to follow these steps:

1. **Upload the new version of MOA to the OpenMOA Dropbox.**
...
2. **Update `invoke.yml`'s `moa_url` to point to the new version of MOA.**
   ...
   This file tells openmoa where to download the `moa.jar` from during the
   packaging process.
...
5. **Verify that openmoa is pointing to the new version of MOA by running:**

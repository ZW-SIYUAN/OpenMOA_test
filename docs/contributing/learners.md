...
another learner to OpenMOA. Before doing this, you should have read the
...
- Classifiers go in `src/openmoa/classifier`.
- Regressors go in `src/openmoa/regressor`.
- Anomaly detectors go in `src/openmoa/anomaly`.
- Semi-supervised classifiers go in `src/openmoa/ssl/classifier`.
...
For example, to add a new classifier class called `MyNewLearner`, you should implement it in `src/openmoa/classifier/_my_new_learner.py` and add it to the `src/openmoa/classifier/__init__.py` file. The `__init__.py` will look like this:
...
```python
from openmoa.classifier import MyNewLearner
```
...
- {py:class}`openmoa.base.Classifier` for classifiers.
- {py:class}`openmoa.base.Regressor` for regressors.
- {py:class}`openmoa.base.AnomalyDetector` for anomaly detectors.
- {py:class}`openmoa.base.ClassifierSSL` for semi-supervised classifiers.
- {py:class}`openmoa.base.Classifier` for online continual learning classifiers.
  Optionally also inheriting from {py:class}`openmoa.ocl.base.TaskAware` or
  {py:class}`openmoa.ocl.base.TaskBoundaryAware` to support learners that are aware of
  task identities or task boundaries.

If your method is a wrapper around a MOA learner, you should use the appropriate
base class:

- {py:class}`openmoa.base.MOAClassifier` for classifiers.
- {py:class}`openmoa.base.MOARegressor` for regressors.
- {py:class}`openmoa.base.MOAAnomalyDetector` for anomaly detectors.

## How do I test my new learner?

You should add a test to ensure your learner achieves and continues to achieves
the expected performance in future versions. OpenMOA provides parametrized
